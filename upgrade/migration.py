...
class MigrationRunner:
    ...
    def create_initial_state(self):
        ...
        if self.manifest.initial_setup_script:
            script_path = self.env.get_script(self.manifest.initial_setup_script)
            logger.info("Running initial setup script: %s", self.manifest.initial_setup_script)
            run_db(initial_db)
            if script_path.endswith('.sql'):
                run_sql_script(initial_db, script_path)
            else:
                run_shell_script(initial_db, script_path)
            stop_db(initial_db)
